...
class Website:
	...
	def getMenu(self,page):
		pattern = re.compile('Menu:&nbsp;</span><a class="fl" href="http://www.google.co.uk/url\?url=(.*?)&amp')
		menu = re.findall(pattern, page)
		return menu
...
if __name__ == "__main__":
	...
	while l:
		...
		menu = spider.getMenu(page)
		print(menu)
		if menu:
			menuIndex += 1
			f2.write(str(menuIndex) + '*' + str(spider.listIndex) + '*' + l.strip('\n') + '*' + unquote(('').join(menu)) + '\n')
		# f2.writelines(page)
